...
public:
    ...
    void insert(ValueType value, KeyType key) {
        heap.template emplace_back(std::make_pair(value, key));
        key2idx[key] = n++;
        CHECK_EQ(heap.size(), n);
...
//        shift_down(cur);
    ...
    void decrease_value(KeyType key, ValueType d = 1) {
        if (d == 0) return;
    ...
    void increase_value(KeyType key, ValueType d = 1) {
        if (d == 0) return;
        IdxType cur = key2idx[key];
        CHECK(cur < n && heap[cur].second == key) << "key not found";
